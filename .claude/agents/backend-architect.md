...
const supabase = createClient<Database>(
  ...
  process.env.SUPABASE_SERVICE_KEY!  // 使用 service_key 绕过 RLS（后台任务）
)
...
容器启动时 Supabase/数据库可能尚未完全就绪：

```javascript
// Supabase Client 健康检查
import { createClient } from '@supabase/supabase-js'
...
> **注意**：也可创建专门的 `_health` 表或使用 Supabase 的健康检查端点。
