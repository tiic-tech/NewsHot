...
### Step 0：判断部署平台（必须先读TECH_SPEC.md）

**如果 TECH_SPEC.md 指定 Vercel 部署**：
- 跳过 Docker/K8s/IaC 部分
- 进入 Vercel 部署流程（见 Step 1V）

**如果 TECH_SPEC.md 指定 Docker/K8s/云服务器部署**：
- 进入传统部署流程（见 Step 1D）
...
**Vercel 自动化检查清单**：
- [ ] 项目已连接 Vercel（GitHub Integration）
- [ ] `vercel.json` 配置正确（Cron Jobs、regions、functions）
...
|------|------|----------------|
| Git Integration | Vercel Dashboard | 项目已连接 |
| Preview Deploy | PR 创建 | Preview URL 可访问 |
| Production Deploy | 合并到 main | Production URL 可访问 |
| Cron Jobs | Vercel Dashboard | Cron 配置显示 |
| Health Check | curl /api/health | API 响应 200 |

---

### Step 1D：传统部署流程（Docker/K8s）
```bash
# 分析当前基础设施与部署需求
# 复核应用架构与扩缩容要求
# 评估安全与合规要求
