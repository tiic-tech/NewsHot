...
| 前端框架 | [如 Next.js 14 + TypeScript] | [理由] |
| 数据库 | [如 Supabase PostgreSQL + pgvector] | [理由] |
| 缓存 | [如 Upstash Redis（Vercel原生）] | [理由，若不需要则注明] |
| 向量存储 | [如 pgvector（Supabase内置）] | [理由] |
| 认证方案 | [如 JWT + bcrypt] | [理由] |
| 部署方式 | [如 Vercel全栈部署] | [理由] |
...
| `SUPABASE_ANON_KEY` | Supabase 公开密钥 | `eyJ...` |
| `SUPABASE_SERVICE_KEY` | Supabase 服务密钥（绑过RLS） | `eyJ...` |
| `DATABASE_URL` | 数据库连接串（直连） | `postgresql://user:pass@db.xxx.supabase.co:5432/postgres` |
...
| [field] | TEXT | NOT NULL | — | [说明] |
| [field] | BOOLEAN | NOT NULL | FALSE | [说明] |
| [field] | DECIMAL(10,2) | NOT NULL | 0.00 | [说明，单位] |
| [field] | vector(384) | — | — | 向量嵌入（384维示例） |
...
```sql
PRIMARY KEY (id)
CREATE INDEX idx_{table}_{field} ON {table} ({field})           -- 原因：[为什么建这个索引]
CREATE UNIQUE INDEX uk_{table}_{field} ON {table} ({field})     -- 原因：[业务唯一性要求]
-- 向量索引（HNSW推荐）
CREATE INDEX idx_{table}_embedding ON {table} USING hnsw ({field}_embedding vector_cosine_ops);
```

**关联关系**：
- `{field}_id` → `{other_table}.id`（N:1，[关系描述]）

**RLS 策略**（Supabase必须）：
```sql
ALTER TABLE {table} ENABLE ROW LEVEL SECURITY;
CREATE POLICY "{table}_select_public" ON {table} FOR SELECT USING (true);
```
