# 产品需求文档（PRD）
> 版本: 1.1 | 作者: product-manager | 日期: 2026-05-11
> ⚠️ 标注 [假设] 的内容需产品确认
...
- **完整流水线**：新闻→摘要→文章→视频创作口播稿→配合口播稿的HTML文件（聚焦视频生成之前的链路）
...
| F11 | 数据源Bullet List展示 | P0 必做 | 前端页面展示抓取的数据源，bullet list形式，包含publish_time, author_name, platform, title, abstract, core_insights, raw_url，无点击跳转 |
| F12 | 文章展示页面 | P0 必做 | 聚合当天所有新增数据源后撰写的Markdown文章展示页面，复用bullet list组件，支持点击跳转查看完整文章 |
...
### US04：数据源Bullet List展示（对应 F11）

**故事**：作为自媒体创作者，我希望在前端页面查看抓取的所有数据源，以便快速浏览当天AI新闻来源。

**验收标准**：
- [ ] 场景1：给定用户打开数据源页面时，当数据源已加载，则展示bullet list形式的数据源列表
- [ ] 场景2：给定每个bullet item时，当展示内容时，则包含publish_time, author_name, platform, title, abstract, core_insights, raw_url字段
- [ ] 场景3：给定bullet item展示时，当用户查看时，则无点击跳转功能
- [ ] 性能：数据源列表加载 < 3秒
- [ ] 性能：bullet item渲染 < 100ms/条

### US05：文章展示与查看（对应 F12）

**故事**：作为自媒体创作者，我希望查看聚合当天所有新增数据源后撰写的Markdown文章，以便预览和审核生成的内容。

**验收标准**：
- [ ] 场景1：给定用户打开文章页面时，当文章已生成，则展示Markdown格式的文章内容
- [ ] 场景2：给定文章列表展示时，当使用bullet list组件时，则复用数据源页面的bullet list组件样式一致
- [ ] 场景3：给定用户点击bullet item时，当点击操作触发，则跳转到完整页面查看完整文章
- [ ] 性能：文章列表加载 < 3秒
- [ ] 性能：完整文章页面加载 < 2秒
...
- [假设] 虽然无并发压力，但是要仔细分析整个管道的异步需求
- [假设] 默认中文输出，但需要后端提供语言切换API，前端可以通过勾选不同语言，来同时输出多份成果
- [待确认] .env.local 字段占位，等待用户自行填写
- [待确认] 按照2个Cron配置，等待用户处理付费
- [待确认] 优先满足OpenAI和Anthropic的LLMAdapter。后端提供API，前端可以手动配置API类型(OpenAI | Anthropic)，base_url, api_key, 模型选择，并通过apply/确认按钮来确认配置
- [待确认] 必须有前端的设计文档
- [待确认] Cluster聚合算法细节阅读grabout文档获取
- [待确认] Cluster聚合算法细节交给后续architect agent处理
...
| LLMAdapter | LLM适配器，支持OpenAI和Anthropic两种API类型的统一接口 |
...
| 阶段 | 目标 | 时间 | 交付物 |
|------|------|------|--------|
| Phase 1: MVP | 验证阶段1完整流水线（新闻聚合+摘要生成+预览审核+文章生成+口播稿） | 3-4周 | Vercel部署demo，完整流程验证 |
...
| Phase 4: 视频生成及发布 | 视频创作口播稿+HTML文件生成 | 后续迭代 | 口播稿生成+HTML模板 |
